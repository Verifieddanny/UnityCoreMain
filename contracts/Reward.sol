...
// // import "./Loanprotocol.sol";
// // contract RewardsContract is ReentrancyGuard {
// //     using SafeMath for uint256;

// //     UnityCoreLendingProtocol private coreLendingProtocol;

// //     constructor(address lendingProtocolAddress) payable {
// //         coreLendingProtocol = UnityCoreLendingProtocol(payable(lendingProtocolAddress));
// //     }
...
// function calculateRewardBasedOnSelectedCollateral()
// public
// view
// returns (uint256)
// {
...
//     return
//         calculateReward(
//             userCollateralInfo[msg.sender].depositFromTSofCORE,
//             userCollateralInfo[msg.sender].coreBalance
//         );
// } else if (
...
//     return
//         calculateReward(
//             userCollateralInfo[msg.sender].depositFromTSofUSDT,
//             userCollateralInfo[msg.sender].usdtBalance
//         );
// } else if (
...
//     return
//         calculateReward(
//             userCollateralInfo[msg.sender].depositFromTSofUSDC,
//             userCollateralInfo[msg.sender].usdcBalance
//         );
// } else {
//     revert("");
// }
...
// //         // Pass the user's address to calculate the reward
// //         UnityCoreLendingProtocol.UserBalance memory userBalance = coreLendingProtocol.getUserCollateralInfo(msg.sender);
// //         uint256 usdtReward = coreLendingProtocol.calculateRewardBasedOnUSDT();
// //         require(usdtReward > 0, "No rewards to claim");
// //         require(!userBalance.userHasClaimedRewards, "Rewards already claimed");
// //         require(coreLendingProtocol.userCollateralInfo(msg.sender).usdtBalance > 0, "No USDT deposit found");
// //         coreLendingProtocol.userBalances(msg.sender).rewardBalancesUSDT += usdtReward;
// //         require(coreLendingProtocol.ucoreToken().transfer(msg.sender, usdtReward), "Reward transfer failed");
// //         coreLendingProtocol.userCollateralInfo(msg.sender).depositFromTSofUSDT = 0;
// //         coreLendingProtocol.userBalances(msg.sender).userHasClaimedRewards = true;
// //         emit coreLendingProtocol.UsdtrewardCLiamed(msg.sender, usdtReward);
// //     }

// //     function ClaimUSDCReward() external payable {
// //         // Pass the user's address to calculate the reward
// //         uint256 usdcReward = coreLendingProtocol.calculateRewardBasedOnUSDC(msg.sender);
// //         require(usdcReward > 0, "No rewards to claim");
// //         require(!coreLendingProtocol.userBalances(msg.sender).userHasClaimedRewards, "Rewards already claimed");
// //         require(coreLendingProtocol.userCollateralInfo(msg.sender).usdcBalance > 0, "No USDC deposit found");
// //         coreLendingProtocol.userBalances(msg.sender).rewardBalancesUSDC += usdcReward;
// //         require(coreLendingProtocol.ucoreToken().transfer(msg.sender, usdcReward), "Reward transfer failed");
// //         coreLendingProtocol.userCollateralInfo(msg.sender).depositFromTSofUSDC = 0;
// //         coreLendingProtocol.userBalances(msg.sender).userHasClaimedRewards = true;
// //         emit coreLendingProtocol.UsdcrewardCLiamed(msg.sender, usdcReward);
// //     }

// //     function monitorLiquidationStatus() external {
// //                 for (uint256 i = 0; i < UnityCoreLendingProtocol.depositors.length; i++) {
// //                     address user = UnityCoreLendingProtocol.depositors[i];
// //                     if (UnityCoreLendingProtocol.isBorrower[user]) {
// //                         uint256 collateralValue = UnityCoreLendingProtocol.calculateCollateralValue();
// //                         uint256 borrowedAmount;

// //                         if (UnityCoreLendingProtocol.userBalances[msg.sender].selectedCollateral == UnityCoreLendingProtocol.CollateralType.CORE) {
// //                             borrowedAmount = UnityCoreLendingProtocol.userBalances[user].coreborrowBalance;
// //                         } else if (UnityCoreLendingProtocol.userBalances[msg.sender].selectedCollateral == UnityCoreLendingProtocol.CollateralType.USDT) {
// //                             borrowedAmount = UnityCoreLendingProtocol.userBalances[user].usdtborrowBalance;
// //                         } else if (UnityCoreLendingProtocol.userBalances[msg.sender].selectedCollateral == UnityCoreLendingProtocol.CollateralType.USDC) {
// //                             borrowedAmount = UnityCoreLendingProtocol.userBalances[user].usdcborrowBalance;
// //                         } else {
// //                             revert("Invalid collateral type");
// //                         }

// //                         uint256 ltvRatio = borrowedAmount*(10000)/(collateralValue);
// //                         if (ltvRatio >= UnityCoreLendingProtocol.liquidationThreshold) {
// //                             uint256 feeDecimal = 45e16; // 4.5% fee in decimal form (0.045)
// //                             uint256 feeAmount = collateralValue*(feeDecimal)/(1e19);
// //                             uint256 remainingCollateral = collateralValue - (feeAmount);
// //                             UnityCoreLendingProtocol.userBalances[user].isDepositFrozen = true;

// //                             if (remainingCollateral > 0) {
// //                                 if (UnityCoreLendingProtocol.userBalances[msg.sender].selectedCollateral == UnityCoreLendingProtocol.CollateralType.USDT) {
// //                                     UnityCoreLendingProtocol.userCollateralInfo[user].usdtBalance = remainingCollateral/(UnityCoreLendingProtocol.usdtPrice);
// //                                 } else if (UnityCoreLendingProtocol.userBalances[msg.sender].selectedCollateral == UnityCoreLendingProtocol.CollateralType.USDC) {
// //                                     UnityCoreLendingProtocol.userCollateralInfo[user].usdcBalance = remainingCollateral/(UnityCoreLendingProtocol.usdcPrice);
// //                                 } else if (UnityCoreLendingProtocol.userBalances[msg.sender].selectedCollateral == UnityCoreLendingProtocol.CollateralType.CORE) {
// //                                     UnityCoreLendingProtocol.userCollateralInfo[user].coreBalance = remainingCollateral/(UnityCoreLendingProtocol.core_price);
// //                                 } else {
// //                                     revert("Invalid collateral type");
// //                                 }
// //                             }
...
// //     function calculateBorrowLimitPercentage(address user, UnityCoreLendingProtocol.CollateralType collateral) internal view returns (uint256) {
...
// // function calculateBorrowedAmountInPrice(address user, UnityCoreLendingProtocol.CollateralType collateral) internal view returns (uint256) {
// //     // Get the borrowed amount for the selected collateral in USD
// //     uint256 borrowedAmountUSD;
// //     if (collateral == UnityCoreLendingProtocol.CollateralType.CORE) {
// //         borrowedAmountUSD = UnityCoreLendingProtocol.userBalances[user].coreborrowBalance * UnityCoreLendingProtocol.core_price / 1e18;
// //     } else if (collateral == UnityCoreLendingProtocol.CollateralType.USDT) {
// //         borrowedAmountUSD = UnityCoreLendingProtocol.userBalances[user].usdtborrowBalance * UnityCoreLendingProtocol.usdtPrice / 1e6;
// //     } else if (collateral == UnityCoreLendingProtocol.CollateralType.USDC) {
// //         borrowedAmountUSD = UnityCoreLendingProtocol.userBalances[user].usdcborrowBalance * UnityCoreLendingProtocol.usdcPrice / 1e6;
// //     } else {
// //         revert("Invalid collateral type");
// //     }
...
contract UnityCoreLendingProtocol is ReentrancyGuard {
    ...
    function withdrawCore(uint256 amount) external payable nonReentrant {
        ...
        if (userBalances[msg.sender].isDepositFrozen) {
            ...
            uint256 remainingBalance = userCollateralInfo[msg.sender]
                .coreBalance
                .sub(feeAmount);
    ...
    function calculateRewardBasedOnCORE() public view returns (uint256) {
        uint256 depositTimestamp = userCollateralInfo[msg.sender]
            .depositFromTSofCORE;
