...
contract WithdrawContract {
    ...
    function withdrawCORE(uint256 amount) external payable {
        ...
        if (userBalance.isDepositFrozen == true) {
    ...
    function calculateAPY(address user) internal view returns (uint256) {
        UnityCoreLendingProtocol.UserBalance memory userBalance = mainContract
            .getUserBalance(user);
        UnityCoreLendingProtocol.UserCollateralInfo
            memory userCollateralInfo = mainContract.getUserCollateralInfo(
        ...
        uint256 totalDeposit = userCollateralInfo.usdtBalance +
            userCollateralInfo.usdcBalance +
            userCollateralInfo.coreBalance;

        // Calculate total borrow balance
        uint256 totalBorrow = userBalance.usdtborrowBalance +
            userBalance.usdcborrowBalance +
            userBalance.coreborrowBalance;

        // Calculate net APY based on the user's deposit and borrow balances
        uint256 netAPY = 0;
        if (totalDeposit > 0) {
            netAPY = (totalBorrow * 10000) / totalDeposit; // To convert to percentage with 2 decimals (APY * 100)
        }
